...
\begin{titlepage}
    ...
    % Sectiunea cu Link-ul GitHub adaugat
    {\large \url{https://github.com/stolniceanuiustin/utcn_opengl_demo}} \par
    \vspace{0.5cm}
...
\begin{itemize}
    ...
    \item \textbf{Obiecte:} Nanosuit \cite{model_nanosuit}, Mario \cite{model_mario}, Creeper \cite{model_creeper}, Zombie \cite{model_zombie}, Bonfire \cite{model_bonfire} și un Cort \cite{model_tent}.
...
\begin{figure}[H]
    ...
    \caption{Vizualizarea scenei cu lanterna (spotlight) activata.}
\end{figure}
...
\textbf{Rezolvarea mea:} Am utilizat tehnica \textit{Billboarding}. Fiecare copac este reprezentat prin 3 planuri texturate care se intersecteaza la un unghi de 60°. Aceasta structura ofera iluzia de volum din orice unghi de vizualizare orizontal, utilizand doar 12 vertex-uri.
...
\subsubsection{3. Ceata pe Skybox}
Pentru a integra vizual skybox-ul cu restul scenei, în special în condiții de ceață, a fost necesară aplicarea unui efect similar și pe acesta. Deoarece skybox-ul este redat ca fiind la o distanță infinită, nu se poate folosi adâncimea pentru a calcula intensitatea ceții.

Soluția implementată în fragment shader-ul skybox-ului (`skyboxShader.frag`) utilizează coordonata verticală a texturii (`textureCoordinates.y`) pentru a modula efectul. Ideea este că ceața este mai densă la orizont (unde `y` este aproape de 0) și inexistentă în zenit (la polul superior al cubemap-ului, unde `y` este 1).

\begin{enumerate}
    \item Se calculează un factor de ceață (`factor`) pornind de la `1.0 - abs(textureCoordinates.y)`. Acest calcul returnează 1.0 la orizont și scade spre 0 la poli.
    \item Factorul este ridicat la o putere pentru a controla finețea tranziției și este multiplicat cu densitatea globală a ceții, asigurând consistența cu ceața aplicată pe restul obiectelor.
    \item Culoarea finală este un amestec (interpolare liniară cu `mix`) între culoarea originală a skybox-ului și culoarea ceții, pe baza factorului calculat.
\end{enumerate}

\begin{minted}[frame=lines, framesep=2mm, fontsize=\small]{glsl}
#version 410 core
...
\texttt{7, 8} & Modificare factor de ceață \\ \hline
\texttt{Q, E} & Rotire robot Nanosuit \\ \hline
\texttt{J, L} & Mișcare sursă de lumină (soare) \\ \hline
\end{tabular}
\end{table}
