...
\textbf{Problema:} Definirea unei traiectorii automate fluide.

\textbf{Rezolvarea mea:} Am implementat o traiectorie fluida bazata pe curba \textit{Catmull-Rom Spline} \cite{catmullrom}, care asigura trecerea prin punctele de control.

\begin{minted}[frame=lines, framesep=2mm, fontsize=\small]{cpp}
glm::vec3 getSplinePoint(float t, std::vector<glm::vec3> points) {
    int p1 = (int)t % points.size();
    int p2 = (p1 + 1) % points.size();
    // ... calcul coeficienti Catmull-Rom ...
    return 0.5f * (points[p0] * q1 + points[p1] * q2 + 
                   points[p2] * q3 + points[p3] * q4);
}
\end{minted}
